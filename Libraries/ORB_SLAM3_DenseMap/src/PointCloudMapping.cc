...
void PointCloudMapping::viewer()
{
    ...
    // std::cout << "Initialize pcl viewer" << '\n';

    // Manage Real Time Display freqency 
    int updateCount = 0;
    // KeyFrame * pCurKF;
    while (1)
    {
        ...
        std::list<KeyFrame *> lNewKeyFrames;
        {
            ...
            if(N == 0)
            {
                // std::cout << "** mlNewKeyFrames size is Zero **" << '\n';
                continue;
            }
            else
            {
        ...
        // gettimeofday(&start,NULL);
        // std::cout << "pcl globalMap size: " << globalMap->size() << std::endl;
